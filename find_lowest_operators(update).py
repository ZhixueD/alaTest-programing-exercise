...
class FindOperator:
    ...
    def find_lowest_price(self, phone_num: str) -> Union[str, List[Tuple[str, float]]]:
        ...
        for operator in list(self.prices_dict):
            ...
            )  # get a list contain all the prefix in one operator

            # filter the prefixes_list only phone_num start with prefix
            prefixs_list = [x for x in prefixs_list if phone_num.startswith(x)]

            # if we find some prefix is the beginning part of the phone number, sort the prefixs_list by prefix length
            if len(prefixs_list) > 0:
                prefixs_list.sort(key=lambda m: len(m))
                prefix_select = prefixs_list[-1]  # choose the longest prefix
